...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>RAWAT INAP - Kepatuhan Verifikasi DPJP terhadap Catatan Komunikasi</h1>
        </div>

        <div class="section-body">
            <div class="card">
                <div class="card-header">
                    <h4><i class="fas fa-chart-line"></i>Grafik Kepatuhan Verifikasi DPJP terhadap Catatan Komunikasi </h4>
                </div>

                <div class="card-body">
                    <form action="{{ route('dpjps.grafik_dp') }}" method="GET" class="d-inline-block">
                    ...
                    <div class="mt-4">
                        <canvas id="dpjpChart" style="height: 400px; width: 100%;"></canvas>
                    </div>

                    <!-- Menampilkan total obat formularium nasional dan total item -->
                    <div class="mt-4">
                        <h5>Total Kepatuhan Verifikasi DPJP terhadap Catatan Komunikasi per Bulan</h5>
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Bulan</th>
                                    <th>Total Berkas Teverivikasi</th>
                                    <th>Total Berkas Tidak Terverifikasi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($dataPerBulan as $bulan => $data)
                                    <tr>
                                        <td>{{ $bulan }}</td>
                                        <td>{{ $data['totalterverifikasi'] }}</td>
                                        <td>{{ $data['totaltidak_terverifikasi'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    var ctx = document.getElementById('dpjpChart').getContext('2d');

        ...
        <div class="section-body">
            <div class="card">
                <div class="card-header">
                    <h4><i class="fas fa-exam"></i> Review bulanan OK UNIT</h4>
                </div>
                <div class="card-body">
                        ...
                        <table class='table table-bordered'>
                            ...
                            <tbody>
                                @php
                                    $totalPenandaan = 0;
                                    $totalPenandaan1 = 0;
                                    $totalPenandaanKeseluruhan = 0;
                                @endphp
                                @foreach ($oks as $item)
                                    @php

                                        // Menghitung total Penandaan
                                        if ($item->penandaan_lokasi_op == '✔️') {
                                            $totalPenandaan++;
                                        }
                                        if ($item->penandaan_lokasi_op == '❌') {
                                            $totalPenandaan1++;
                                        }

                                        $totalPenandaanKeseluruhan = $totalPenandaan + $totalPenandaan1;
                                    @endphp
                                    <tr>
                                        <td>{{ $item->tanggal }}</td>
                                        <td>{{ $item->no_rm }}</td>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="11"><strong><center>Total Penandaan OP: {{ $totalPenandaan }}</center></strong></td>
                                    <td colspan="17"></td>
                                </tr>
                                <tr>
                                    @if($totalPenandaanKeseluruhan > 0)
                                        @php
                                            $persentasePenandaan = ($totalPenandaan / $totalPenandaanKeseluruhan) * 100;
                                        @endphp
                                        <td colspan="11"><strong><center>Persentase Penandaan OP: {{ number_format($persentasePenandaan, 2) }}%</center></strong></td>
                                    @else
                                        <td colspan="11"><strong><center>Persentase Penandaan OP: 0%</center></strong></td>
                                    @endif
                                </tr>
                            </tfoot>
                        </table>

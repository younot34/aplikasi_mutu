...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Farmasi - Formularium Nasional</h1>
        </div>

        <div class="section-body">
            <div class="card">
                <div class="card-header">
                    <h4><i class="fas fa-chart-line"></i>Grafik Formularium Nasional </h4>
                </div>

                <div class="card-body">
                    <form action="{{ route('farmasis.grafik_fornas') }}" method="GET" class="d-inline-block">
                    ...
                    <div class="mt-4">
                        <canvas id="farmasiChart" style="height: 400px; width: 100%;"></canvas>
                    </div>

                    <!-- Menampilkan total obat formularium nasional dan total item -->
                    <div class="mt-4">
                        <h5>Total Obat Formularium Nasional dan Total Item per Bulan</h5>
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Bulan</th>
                                    <th>Total Obat Formularium Nasional</th>
                                    <th>Total Item</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($chartData['labels'] as $index => $bulan)
                                    <tr>
                                        <td>{{ $bulan }}</td>
                                        <td>{{ $chartData['totalObatFornas'][$index] }}</td>
                                        <td>{{ $chartData['totalItem'][$index] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    var ctx = document.getElementById('farmasiChart').getContext('2d');

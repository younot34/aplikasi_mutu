...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>PPI - Kepatuhan APD</h1>
        </div>

        <div class="section-body">
            <div class="card">
                <div class="card-header">
                    <h4><i class="fas fa-chart-line"></i> Grafik Kepatuhan APD</h4>
                </div>

                <div class="card-body">
                    <form action="{{ route('apds.grafik_apd_tahunan') }}" method="GET" class="d-inline-block">
                    ...
                    <canvas id="apdChart" class="mt-4" style="height: 400px; width: 100%;"></canvas>

                    <!-- Menampilkan total hasil akhir per bulan -->
                    <div class="mt-4">
                        <h5>Total APD Ya dan Tidak per Bulan</h5>
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($chartData['labels'] as $index => $bulan)
                                    <tr>
                                        <td>{{ $bulan }}</td>
                                        <td>{{ $chartData['totalYa'][$index] }}</td>
                                        <td>{{ $chartData['totalTidak'][$index] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    var ctx = document.getElementById('apdChart').getContext('2d');
    var chart = new Chart(ctx, {
        type: 'line', // Tipe grafik garis
        data: {
            labels: @json($chartData['labels']), // Nama bulan
            datasets: [{
                label: 'Kepatuhan APD Bulanan (%)',

    ...
    <section class="section">
        ...
        <div class="section-body">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('plebitis.update', $plebiti->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            <label>Judul</label>
                            <input type="text" name="judul" value="{{ $plebiti->judul }}" class="form-control @error('judul') is-invalid @enderror" required>
                            @error('judul')
                        ...
                        <div class="form-group">
                            ...
                            <select id="monthSelector" name="bulan" class="form-control" onchange="updateDates()">
                                @foreach (['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'] as $index => $month)
                                    <option value="{{ $index + 1 }}" {{ $plebiti->bulan == ($index + 1) ? 'selected' : '' }}>{{ $month }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th colspan="35" class="text-center">Judul: <span id="judulDisplay">{{ $plebiti->judul }}</span></th>
                                    </tr>
                                    <tr>
                                ...
                                <tbody id="dataBody">
                                    <tr>
                                        <td rowspan="2">
                                            <input type="text" name="variabel" value="{{ $plebiti->variabel }}" class="form-control" required>
                                        </td>
                                        <td>
                                            <input type="text" name="sub_variabel_1" value="{{ $plebiti->sub_variabel_1 }}" class="form-control" required>
                                        </td>
                                        <td id="datesColumn1" class="d-flex justify-content-start"></td>
                                        <td><input type="number" name="total_1" id="total_1" value="{{ $plebiti->total_1 }}" readonly></td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <input type="text" name="sub_variabel_2" value="{{ $plebiti->sub_variabel_2 }}" class="form-control" required>
                                        </td>
                                        <td id="datesColumn2" class="d-flex justify-content-start"></td>
                                        <td><input type="number" name="total_2" id="total_2" value="{{ $plebiti->total_2 }}" readonly></td>
                                        <td rowspan="2">
                                            <div style="position: relative;">
                                                <input type="number" name="sasaran" value="{{ $plebiti->sasaran }}" pattern="\d*" required style="padding-right: 30px;">
                                                <span style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%);">‰</span>
                                            </div>
                                        </td>
                                        <td>
                                            <div style="position: relative;">
                                                <input type="number" name="hasil" id="hasil" value="{{ $plebiti->hasil }}" readonly style="padding-right: 30px;">
                                                <span style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%);">‰</span>
                                            </div>
                                        </td>
                                    </tr>
...
<script>
    ...
    function calculateTotalsAndResult() {
        ...
        if (total2 !== 0) {
            const result = (total1 / total2) * 1000;
